...
const supportedLanguages = [
  { title: 'Chinese', type: 'zh' },
  { title: 'French', type: 'fr' },
  { title: 'German', type: 'de' },
  { title: 'Italian', type: 'it' },
  { title: 'Spanish', type: 'es' },
];

const Language = ({transcriptionId}: {transcriptionId: string}) => {
  ...
  const handleLanguageSelect = (selectedLanguage: string) => {
    const selectedOptionData = supportedLanguages.find(v => v.title === selectedLanguage);
    if (selectedOptionData) {
      setState({
        selectedType: selectedOptionData.type,
        selectedLanguage: selectedOptionData.title,
        isSidebarOpen: true
      })
    }
  };
  ...
  return (
    <>
      <Dropdown
        options={supportedLanguages.map((v) => v.title)}
        selectedOption={supportedLanguages.find(v => v.type === state.selectedType)?.title || ''}
